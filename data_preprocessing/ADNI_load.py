...
diagnosis = np.nan_to_num(np.array(diagnos_org["DXCHANGE"])) + np.nan_to_num(np.array(diagnos_org["DXCURREN"])) + np.nan_to_num(np.array(diagnos_org["DIAGNOSIS"]))     # noqa
...
meta_data_final = meta_data_final[meta_data_final["RID"].isin(RID)].reset_index()       # noqa
...
for a, _ in enumerate(meta_data_final["USERDATE"]):
    Age[a] = int(np.array(meta_data_final["USERDATE"][a][0:4])) - meta_data_final["PTDOBYY"][a]     # noqa
