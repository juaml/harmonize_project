...
save_dir = '/home/nnieto/Nico/Harmonization/data/final_data_split_TIV/'
TIV_percentage = 20

for site in sites:

    Y = pd.read_csv(root_dir + "Y_" + site + ".csv")
    X = pd.read_csv(root_dir + "X_" + site + ".csv")
    ...
    # Save new files
    # X.to_csv(save_dir + "X_" + site + ".csv", index=False)
    # Y.to_csv(save_dir + "Y_" + site + ".csv", index=False)
