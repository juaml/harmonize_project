...
x1 = pd.read_csv(data_dir+'X_CamCAN.csv')
x1 = x1.iloc[:, 0:features]
y1 = pd.read_csv(data_dir+'Y_CamCAN.csv')['age']
site1 = pd.read_csv(data_dir+'Y_CamCAN.csv')['site']
...
print("MAE Original: " + str(np.mean(np.abs(results_original["y_pred"] - results_original["y_true"]))))             # noqa
print("MAE UnHarmonize: " + str(np.mean(np.abs(results_unharmonize["y_pred"] - results_unharmonize["y_true"]))))    # noqa

print("MAE Cheat: " + str(np.mean(np.abs(results_Cheat["y_pred"] - results_Cheat["y_true"]))))                      # noqa

print("MAE Leak: " + str(np.mean(np.abs(results_leakage["y_pred"] - results_leakage["y_true"]))))                   # noqa
print("MAE JuHarmonize: " + str(np.mean(np.abs(results_JuHarmonize["y_pred"] - results_JuHarmonize["y_true"]))))    # noqa
...
plot_list = [[results_Cheat, "Cheat"],
             [results_JuHarmonize, "JuHarmonize"],
             [results_original, "Original Data"],
             [results_unharmonize, "None"],
             [results_leakage, "Leakage"]]

plt.figure(figsize=[10, 7])

for data_harm, harmo_mode in plot_list:
    data = data_naming(data_harm)

    plt.figure(figsize=[10, 7])
    sbn.scatterplot(data=data, x="", y="y_pred", hue="Site")
    plt.title(harmo_mode + " MAE: " + str(round(mae, 4)))
    plt.xlabel("True Age")
